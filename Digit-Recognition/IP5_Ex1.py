...
train = x[:,:80].reshape(-1,400).astype(np.float32) #size = 4000x400
test  = x[:,80:100].reshape(-1,400).astype(np.float32) #size = 1000x400
...
ret,results,neighbors,distance = knn.findNearest(test, k=6)
...
image2 = cv2.imread("num2.png")
gray2 = cv2.cvtColor(image2,cv2.COLOR_BGR2GRAY)
cv2.imshow("Image",image2)
cv2.imshow("Gray",gray2)
cv2.waitKey(0)

blurred = cv2.GaussianBlur(gray2,(3,3),4)
